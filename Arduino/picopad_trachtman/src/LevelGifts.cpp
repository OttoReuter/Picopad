...
void handlePlayerMovement() {
    ...
    if(trachtman.x >= 240) { movingLeft=true; movingRight=false; }
    // Level1
    if(currentLevel == &level1) {
        if(trachtman.y < 85 && trachtman.x >= 240) {
            trachtman.y = 85;
        }
    ...
    if(currentLevel == &level2) {
        if (trachtman.y <85 && (trachtman.x <= 0 || trachtman.x >= 240)) {
            trachtman.y = 85;
        }
    }
    // Level 3
    if(currentLevel == &level3) {
        if (trachtman.y <45 && (trachtman.x <= 0 || trachtman.x >= 240)) {
            trachtman.y = 45;
        }
    }
}
